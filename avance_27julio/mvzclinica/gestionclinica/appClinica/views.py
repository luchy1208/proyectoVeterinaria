...
def vistaGestionarCitasAgendadas(request, cita_id):
    # Resto del código de la vista
    
    return render(request, 'vistaGestionarCitaAgendada.html', {
...
def vistaAgendarCita(request):
    ...
    retorno = {"listaMascotas":mascotas}
    print(retorno)
...
def agendarCita(request):
    if(request.user.is_authenticated):
        ...
        try:
            ...
            # Crear la nueva cita
            cita = AgendarCita(ageCitMascota=mascota, ageCitTipServicios=servicio)
            cita.save()

            return redirect('/vistaLogin',urlReturn="agendarCita")
        
        except Mascota.DoesNotExist:
            # Manejar el caso si no se encuentra la mascota
            return render(request, '/cliente/registrarMascota', {'mensaje': 'La mascota no existe.'})
        
        except Servicios.DoesNotExist:
            # Manejar el caso si no se encuentra el servicio
            return render(request, 'inicioCliente', {'mensaje': 'El servicio no existe.'})
    else:
        # Si la solicitud es GET, mostrar el formulario para agendar la cita
        mascotas = Mascota.objects.all()
        servicios = Servicios.objects.all()
        return render(request, 'frmAgendarCita.html', {'mascotas': mascotas, 'servicios': servicios})
...
def registrarMascota(request):
    ...
    try:
        ...
        mascota = None
        with transaction.atomic():
            # obtener cuantas mascotass se han registrado
            
            #ear = datetime.strptime(nacimientoFecha, '%Y')
            #print (year)
            #month = datetime.strptime(nacimientoFecha, '%m')
            #day = datetime.strptime(nacimientoFecha, '%d')
            #fecha = year + '-' + month + '-' + day
            #mascota = Mascota(fecha=nacimientoFecha)
            ...
            mascota = Mascota(masCliUsuario= usuario,
                masCodigo=codigoMascota,masNombre=nombreMas, masEspecie=especieMas, masRaza=razaMas, masColor=colorMas, masSexo=sexoMas, masFecNacimiento=nacimientoFecha)
            # salvar el elemento en la base de datos
            mascota.save()
            estado = True
            mensaje = f"Mascota registrada correctamente con el codigo {codigoMascota}"
            return redirect("cliente/vistaAgendarCita/")
    except Error as error:
        transaction.rollback()
        mensaje = f"{error}"
